...
# DFG Cleaning
#
def cleanDFG(dfg: Dict[Tuple[str, str], int], df_thresh=0.0, rel_factor=0.01):
    ...
    for a, b in dfg:
        ...
        v = dfg.get((a, b), 0)
        if (
            v >= rel_factor * sum(allOutgoingEdgeValues) / len(allOutgoingEdgeValues)
            or v >= rel_factor * sum(allIncomingEdgeValues) / len(allIncomingEdgeValues)
        ) and v >= df_thresh:
            newDfg[(a, b)] = v
    return newDfg
...
def repairLogSkip(logProcessor: LogProcessor, threshold) -> LogProcessor:
    ...
    for a in skips:
        ...
        if len(canSkip) > 0:
            ...
            logProcessor.getActivityOccurrences()[newNamedTau] = dfCountFromAToOther
...
def repairLogLoop(logProcessor: LogProcessor, df_p1: float) -> LogProcessor:
    ...
    loop_candidates: Set[Tuple[str]] = {
        x for x in bf_approach_reachable if x[len(x) - 1] != END_ACTIVITY
    }
    ...
    for newNamedTau in insertNamedTausBetween:
        ...
        for variant in variantsBefore:
            newVariant = variant.replace(
                newNamedTau[0] + "," + newNamedTau[1],
                newNamedTau[0] + "," + newActName + "," + newNamedTau[1],
            )
...
def buildCandidates(logProcessor: LogProcessor) -> Set[Tuple[Set[str], Set[str]]]:
    ...
    for a in activitiesHat:
        for b in activitiesHat:
            if (
                (a, b) in dfRelation
                and (b, a) not in dfRelation
                and (a, a) not in dfRelation
                and (b, b) not in dfRelation
            ):
    ...
    def satisfiesCndCriteria(c):
        ...
        b_without_a = b - a
        return (
            Utils.isNoDFRelationBetweenAsymmetric(dfRelation, a, a_without_b)
            and Utils.isNoDFRelationBetweenAsymmetric(dfRelation, b_without_a, b)
            and Utils.areAllRelationsDFBetweenNonStrict(dfRelation, a, b)
            and Utils.areNotAllRelationsDF_SWITCHED(
                dfRelation, a_without_b, b_without_a
            )
        )
...
def getCandidateScore(
    ...
    return (
        fittingWithActs / numberOfTracesWithActs >= t
        and min(
            {
                numOfFittingTracesContaingAct[a] / numOfTracesContainingAct[a]
                for a in allActsInCandidate
                if numOfTracesContainingAct[a] > 0
            }
        )
        >= t
...
def pruneCandidatesFitness(
    logProcessor: LogProcessor,
    candidates: Set[Tuple[Set[str], Set[str]]],
    min_fitting_traces: float,
) -> Set[Tuple[Set[str], Set[str]]]:
    return {
        c for c in candidates if getCandidateScore(logProcessor, c, min_fitting_traces)
    }
...
#
def pruneCandidatesMaximal(
    logProcessor: LogProcessor, candidates: Set[Tuple[Set[str], Set[str]]]
) -> Set[Tuple[Set[str], Set[str]]]:
...
candidatePlaceMap: Dict[
    Tuple[FrozenSet[str], FrozenSet[str]], PetriNet.Transition
] = dict()
...
def buildPetriNet(
    ...
    net = PetriNet(
        ...
        transitions=set(candidatePlaceMap.values()),
    )
...
class AlphaPPP(ExperimentAlgorithm):
    ...
    def execute_steps(self, log_processor: LogProcessor, args={}):
        if self.variant.startswith("TAB|"):
            ...
            lp = log_processor
            balance_value = args["balance_value"]
            replay_local_fitness_required = args["replay_local_fitness_required"]
            candidate_pruning_local_fitness_required = args[
                "candidate_pruning_local_fitness_required"
            ]
            if args["loop_repair_threshold"] is not None:
                lp = repairLogLoop(lp, df_p1=args["loop_repair_threshold"])
            if args["skip_repair_threshold"] is not None:
                lp = repairLogSkip(lp, args["skip_repair_threshold"])
        else:
            raise ValueError("Unknown Parameter Mode")

        def copy_cnds(cnd_set):
            if args["copy_candidates_into"] is not None:
                args["copy_candidates_into"].append(cnd_set.copy())

        cnd = set()
        cnd = buildCandidates(lp)
        if args["inject_candidates"] is not None:
            for c in args["inject_candidates"]:
                cnd.add(c)
        copy_cnds(cnd)
        if balance_value < 1.0:
        ...
        # Replay
        replay_res = ReplayProcessor.replayWithAllTraces(
            (net, im, fm), lp.getVariants()
        )
        replay_res_problematic_places = {
            p for p in replay_res if replay_res[p] < replay_local_fitness_required
        }
